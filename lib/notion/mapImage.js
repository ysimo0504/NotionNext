  ...
  const needConvert =
    (!hasConverted &&
      (block.type === 'bookmark' ||
        ...
        ret.includes('images.unsplash.com') ||
        ret.includes('source.unsplash.com') ||
        ret.includes('githubusercontent.com') ||
        ret.includes('googleusercontent.com') ||
        ret.includes('gravatar.com') ||
        ret.includes('p1.qhimg.com') ||
        ret.includes('webmention.io') ||
        ret.includes('ko-fi.com'))) ||
    ret.indexOf('attachment') === 0

  // Notion旧图床
  if (needConvert) {
    // 将需要转换的外链通过本地代理到 /api/img，借此在首访时迁移到 R2
    try {
      const proxied =
        '/api/img?src=' +
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    ...
    if (
      ...
      try {
        ...
        const isR2 = r2Base && ret.startsWith(r2Base)
        if (!isR2) {
