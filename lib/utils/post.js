...
import { replaceBlockMapImagesWithR2 } from '@/lib/images/r2'
...
export async function processPostData(props, from) {
  ...
  if (props.post?.blockMap?.block) {
    // 将文章中的图片统一迁移并替换为 R2 直链，满足“获取后即替换”的要求
    try {
      await replaceBlockMapImagesWithR2(props.post.blockMap, 5)
    } catch (e) {
      console.error('replaceBlockMapImagesWithR2 error:', e)
    }
