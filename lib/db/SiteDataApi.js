...
export async function fetchGlobalAllData({
  ...
  // devLog(`🕒 开始获取全站数据: ${siteIds.length} 个站点`)
  ...
  try {
    for (let index = 0; index < siteIds.length; index++) {
      ...
      // devLog(`✅ 完成站点: ${siteId}，耗时: ${(siteEnd - siteStart)}ms`)
    }
  } catch (error) {
  ...
  // devLog(`🏁 全站数据获取完成，总耗时: ${(globalEnd - globalStart)}ms`)
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  // const siteEnd = Date.now()
  // devLog(`⏱ [Notion API] 站点 ${pageId} 耗时: ${siteEnd - siteStart}ms`)
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // devLog(`[${taskId}] ⏱ 查找文章耗时: ${step2End - step2Start}ms @ ${new Date().toISOString()}`)
  ...
  if (post) {
    ...
    // devLog(`[${taskId}] ⏱ processPostData 耗时: ${step5End - step5Start}ms @ ${new Date().toISOString()}`)
  } else {
    props.post = null
  }
...
async function convertNotionToSiteData(
  ...
  // devLog(`[${traceId}] ⏱ UUID 转换耗时: ${stepEnd1 - stepStart1}ms @ ${new Date().toISOString()}`)
  ...
  // devLog(`[${traceId}] ⏱ normalizeNotionMetadata 耗时: ${stepEnd3 - stepStart3}ms @ ${new Date().toISOString()}`)
  ...
  // devLog(`[${traceId}] ⏱ getAllPageIds 耗时: ${stepEnd5 - stepStart5}ms, count: ${pageIds.length} @ ${new Date().toISOString()}`)
  ...
  // devLog(`[${traceId}] ⏱ 缓存检查耗时: ${stepEnd6 - stepStart6}ms, 需补拉: ${idsNeedFetch.length} @ ${new Date().toISOString()}`)
  ...
  // devLog(`[${traceId}] ⏱ 筛选 + 排序 allPages 耗时: ${stepEnd10 - stepStart10}ms @ ${new Date().toISOString()}`)
