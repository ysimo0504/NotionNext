    ...
    allPages: [
      {
        ...
        summary:
          '访问文档获取帮助 → https://docs.heysimo.com/article/vercel-deploy-notion-next',
        status: 'Published',
        type: 'Post',
        slug: 'oops',
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  post = props?.allPages?.find(p => {
    ...
    return (
      ...
      slugCandidates.has(safeIdToUuid(p.id)) ||
      slugCandidates.has(safeIdToUuid(fullSlug))
    )
  })
...
async function convertNotionToSiteData(
  ...
  if (
    ...
    console.warn(
      `[NotionNext] pageId "${SITE_DATABASE_PAGE_ID}" is not a database (type: ${rawMetadata?.type || 'unknown'}). Returning empty data.`
    )
  ...
  // Fallback: if getPage failed to populate collection_query (swallowed error),
  // manually call getCollectionData for each view and inject results.
  if (
    (!collectionQuery || Object.keys(collectionQuery).length === 0) &&
    collectionId &&
    viewIds?.length > 0
  ) {
    collectionQuery = {}
    for (const viewId of viewIds) {
      try {
        const cv = collectionView?.[viewId]?.value
        ...
        if (result?.result?.reducerResults) {
          collectionQuery[collectionId] = collectionQuery[collectionId] || {}
          collectionQuery[collectionId][viewId] = result.result.reducerResults
          // Merge blocks from the collection data into recordMap
          if (result.recordMap?.block) {
            pageRecordMap.block = {
              ...pageRecordMap.block,
              ...result.recordMap.block
            }
          }
        }
      } catch (e) {
        console.warn(
          '[fallback] getCollectionData failed for view',
          viewId,
          e.message
        )
      }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || ''
  const defaultDescription = NOTION_CONFIG?.DESCRIPTION || ''
