...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  let content = normalizePageBlock(
  ...
  const configTableId = content?.find(contentId => {
    return (
      normalizePageBlock(pageRecordMap.block[contentId].value)?.type ===
      'collection_view'
    )
  })
  ...
  try {
    // 将INLINE_CONFIG合并，@see https://docs.heysimo.com/article/notion-next-inline-config
